...
namespace MauiApp1.Views
{
 ...
 private ObservableCollection<Kategoria> kategorie;
  ...
  private PlikSerwis plikSerwis;
   ...
   public ListaDlaSklepuStrona(ObservableCollection<Kategoria> kat, string sklep, PlikSerwis serwis)
 {
  InitializeComponent();
       kategorie = kat;
  nazwaSklep = sklep;
...
// zebrac produkty dla tego sklepu
...
var posortowane = produktyDlaSklepu.OrderBy(x => x.nazwaKategorii).ThenBy(x => x.produkt.CzyKupiony).ToList();
...
if (kat != null)
 ...
 if (posortowane.Count == 0)
  ...
  { 
  Text = $"Brak produktow dla sklepu: {nazwaSklep}", 
        FontSize = 18,
